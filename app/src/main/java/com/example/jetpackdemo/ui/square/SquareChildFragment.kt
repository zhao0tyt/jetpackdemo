 ...
 * 广场模块的子模块
 */
class SquareChildFragment : BaseFragment<BaseViewModel, IncludeListBinding>(){
    //界面状态管理者
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        loadsir = loadServiceInit(swipeRefreshLayout) {
            //点击重试时触发的操作
            loadsir.showLoading()
            viewModel.getSquareChildData(true)
        }
        //初始化recyclerView
...
//                            })
            ...
            loadMoreModule.setOnLoadMoreListener {
                viewModel.getSquareChildData(false)
            }
        }
    }
    ...
    override fun createObserver() {
        viewModel.squareDataState.observe(viewLifecycleOwner, Observer {
